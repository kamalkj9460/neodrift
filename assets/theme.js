...
$("#groups").change(function () {
  var optionModel = $(this).find('option:selected').attr('data-model');
  var optionUrl = $(this).find('option:selected').attr('data-url');
  var car = $(this).find('option:selected').attr('value');
  //alert(optionModel);
  //alert(car);
  var modelArray = optionModel.split(',');
  var urlArray = optionUrl.split(',');
  $("#sub_groups").find('option').remove();
  for(var i = 0; i < urlArray.length; i++)
  {  
  $("#sub_groups").append("<option value=" +urlArray[i]+ " class=mod-new>" + car + " "+modelArray[i]+ "</option>");
  }
  myFunction();
});
